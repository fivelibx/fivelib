...
async def register(data: RegisterRequest):
    ...
    if user_exists.data:
        ...
        if not update_response.data:
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Erro ao atualizar código de verificação. Tente novamente.")
...
async def verify_code(data: VerifyCodeRequest):
    ...
    if expires_at_str:
        ...
        if datetime.now(timezone.utc) > expires_at:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="O código de verificação expirou. Solicite um novo código.")
    update_response = supabase.table("user").update({
        "is_active": True,
